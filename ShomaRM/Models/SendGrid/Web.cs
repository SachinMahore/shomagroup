...
namespace ShomaRM.Models
{
	public class Web : ITransport
	{
		...
		public void Deliver(ISendGrid message)
		{
			...
			CheckForErrors(response,message);
		}
        ...
        private static void CheckForErr(int response, ISendGrid message)
        {
            var category = "";
           
            try
            {
                Newtonsoft.Json.Linq.JObject json = Newtonsoft.Json.Linq.JObject.Parse(message.Header.JsonString());
                category = json["category"].ToArray()[1].ToString();
            }
            catch
            {
                category = "";
            ...
            switch (response)
            {
                ...
                case 404:
                    ...
                    break;
                case 405:
                    respmsg = "Method No Allowed";
                    break;
                case 413:
                    respmsg = "Payload to Large";
                    return;
                case 415:
                    respmsg = "Unsupported Media Type";
                    break;
                case 429:
                    respmsg = "Too Many request";
                    break;
                case 500:
                    respmsg = "Server Unavailable";
                    break;
                case 503:
                    respmsg = "Service not available";
                    break;
                default:
                    respmsg = "Error";
                    break;
		...
		internal List<KeyValuePair<String, String>> FetchFormParams(ISendGrid message)
		{
			var result = new List<KeyValuePair<string, string>>
			{
                ...
                //    message.date.Length == 0 ? null : message.date),
                //new KeyValuePair<String, String>("date_at",
                //    message.date.Length == 0 ? null : message.date_at),
